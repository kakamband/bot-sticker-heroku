...
client.on('authenticated', (session) => {
    ...
    fs.readFile(__dirname + '/auth_info.json', { encoding: 'utf8' }, function(err, data) {
        if (err) {
            fs.writeFileSync(__dirname + '/auth_info.json', JSON.stringify(session))
            ...
            fs.unlinkSync(__dirname + '/auth_info.json)
            fs.writeFileSync(__dirname + '/auth_info.json', JSON.stringify(session))
            console.log("\n\nToken also saved on a file named session.json in this directory. Please delete this file after copy if you will use enviroment variable.")
            process.exit()
        }
